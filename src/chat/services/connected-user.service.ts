...
export class ConnectedUserService {
    ...
    async findConnectedUsersByUserIds(userIds: string[]): Promise<ConnectedUser[]> {
        ...
        try {
            ...
            this.logger.error(`Error finding connected users by IDs ${userIds.join(', ')}: ${error.message}`, error.stack);
            if (error instanceof WsException) {
                throw error;
            }
            throw new WsException('Failed to retrieve connected users for notification.');
        }
    }
}